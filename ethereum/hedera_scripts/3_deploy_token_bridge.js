...
async function main() {
  ...
  const TokenImplementationAddress = await deployer.deploy(client, "TokenImplementation", TokenImplementation.bytecode, 100000, new ContractFunctionParameters());

  const BridgeSetup = require("../build/contracts/BridgeSetup.json");
  const BridgeSetupAddress = await deployer.deploy(client, "BridgeSetup", BridgeSetup.bytecode, 100000, new ContractFunctionParameters());
  
  const BridgeImplementation = require("../build/contracts/BridgeImplementation.json");
  const BridgeImplementationAddress = await deployer.deploy(client, "BridgeImplementation", BridgeImplementation.bytecode, 100000, new ContractFunctionParameters());
  ...
  await deployer.deploy(client, "TokenBridge", TokenBridge.bytecode, 200000, params);
